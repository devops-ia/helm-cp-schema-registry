...
*/}}
{{- define "cp-schema-registry.patchSelectorLabels" -}}
{{- if not (hasKey ._target "labelSelector") }}
{{- $selectorLabels := (include "cp-schema-registry.selectorLabels" .) | fromYaml }}
...
This template uses the cp-schema-registry.selectorLabels template to add a labelSelector to topologySpreadConstraints if one isn't specified.
This works because Helm treats dictionaries as mutable objects and allows passing them by reference.
*/}}
{{- define "cp-schema-registry.patchTopologySpreadConstraints" -}}
{{- range $constraint := .Values.topologySpreadConstraints }}
{{- include "cp-schema-registry.patchSelectorLabels" (merge (dict "_target" $constraint (include "cp-schema-registry.selectorLabels" $)) $) }}
{{- end }}
{{- end }}
